...
divdata_ww <- xx %>% 
  ...
  mutate(dhi = 1 - hhi,
         ...
         variable2 = case_when(variable == "dayparts" ~ "when", .default = "where")) %>% 
  group_by(id, t, variable2) %>% 
  summarise(log_adspend_x_dhi = mean(log_adspend_x_dhi))

divdata_when <- divdata_ww %>% 
  rename(log_adspend_x_dhi_when = log_adspend_x_dhi) %>% 
  select(-variable2)
summary(divdata_when)

divdata_where <- divdata_ww %>% 
  filter(variable2 == "where") %>%
  rename(log_adspend_x_dhi_where = log_adspend_x_dhi) %>% 
  select(-variable2)
summary(divdata_where)

xx_ww <- xx %>% 
  select( - matches(c("_gen", "_net", "_dpt"))) %>% 
  left_join( divdata_when, by = c("id", "t") ) %>% 
  left_join( divdata_where , by = c("id", "t") ) 
xx_ww %>%  summary
...
aaw_log0_hedo <- bf(log_adawareness0 ~ 0 +
  ...
  lf(f2 ~ 0 + 
       log_clutter +
       log_adspend +
       log_adspend_x_dhi_when + 
       log_adspend_x_dhi_where + 
       log_adspend : hedonic +
       log_adspend_x_dhi_when : hedonic + 
       log_adspend_x_dhi_where : hedonic +
       (0 + log_clutter + log_adspend | id))
...
aaw_log0_risk <- bf(log_adawareness0 ~ 0 +
  ...
  lf(f2 ~ 0 + 
       ...
       log_adspend_x_dhi_where + 
       log_adspend : risk +
       log_adspend_x_dhi_when : risk + 
       log_adspend_x_dhi_where : risk +
       (0 + log_clutter + log_adspend | id))
...
aaw_log0_inv <- bf(log_adawareness0 ~ 0 +
  ...
  lf(f2 ~ 0 + 
       ...
       log_adspend_x_dhi_where + 
       log_adspend : involvement +
       log_adspend_x_dhi_when : involvement + 
       log_adspend_x_dhi_where : involvement +
       (0 + log_clutter + log_adspend | id))
...
aaw_log0_size <- bf(log_adawareness0 ~ 0 +
  ...
  lf(f2 ~ 0 + 
       ...
       log_adspend_x_dhi_where + 
       log_adspend : size +
       log_adspend_x_dhi_when : size + 
       log_adspend_x_dhi_where : size +
       (0 + log_clutter + log_adspend | id))
...
>>>>>>> 951260bb78e9dac8c8ea7d2d4b8ced5a65894b73
divdata_tot <-xx %>%
  select(id, t, log_adspend) %>%
  left_join(divdata) %>%
  mutate(
